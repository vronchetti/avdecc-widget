...
/**
* avdecc_end_station_details.cpp
*
*/

#include "avdecc_end_station_details.h"

wxBEGIN_EVENT_TABLE(avdecc_end_station_details, wxDialog)
EVT_GRID_CELL_CHANGED(avdecc_end_station_details::OnGridChange)
EVT_CHOICE(CHANNEL_CHANGE, avdecc_end_station_details::OnChannelChange)
wxEND_EVENT_TABLE()

avdecc_end_station_details::avdecc_end_station_details(wxWindow *parent,
avdecc_end_station_configuration *end_station_config, avdecc_stream_configuration *stream_config) :
...
wxDefaultPosition, wxDefaultSize)
{
	...
	*/
	m_end_station_config = end_station_config;
...
avdecc_end_station_details::~avdecc_end_station_details() {}

void avdecc_end_station_details::GetEndStationData()
{
...
void avdecc_end_station_details::GetStreamData()
{
...
void avdecc_end_station_details::SetStreamDetails()
{
	for (unsigned int i = 0; i < m_stream_input_count; i++) //initial stream names and channel counts
	{
		struct avdecc_stream_configuration_details m_stream_details;
...
void avdecc_end_station_details::CreateAndSizeEndStationDetailsList()
{
	...
	avdecc_end_station_details_sizer = new wxStaticBoxSizer(wxVERTICAL, this, "End Station Details");
	avdecc_end_station_details_sizer->Add(Sizer1);
	avdecc_end_station_details_sizer->Add(Sizer2);
	avdecc_end_station_details_sizer->Add(Sizer3);
	avdecc_end_station_details_sizer->Add(Sizer4);
	avdecc_end_station_details_sizer->Add(Sizer5);
	avdecc_end_station_details_sizer->Add(Sizer6);
	avdecc_end_station_details_sizer->Add(Sizer7);
}

void avdecc_end_station_details::CreateChannelChoices()
...
void avdecc_end_station_details::SetInputMappings(struct audio_mapping &map)
{
...
void avdecc_end_station_details::SetOutputMappings(struct audio_mapping &map)
{
	int16_t stream_index = 0;
	int16_t stream_channel = 0;
	int16_t cluster_offset = 0;
...
void avdecc_end_station_details::SetInputChannelName(unsigned int stream_index, wxString name)
{
	input_stream_name_grid->SetCellValue(stream_index, 0, name);
}

void avdecc_end_station_details::SetOutputChannelName(unsigned int stream_index, wxString name)
{
	output_stream_name_grid->SetCellValue(stream_index, 0, name);
}

void avdecc_end_station_details::SetInputChannelCount(unsigned int stream_index, size_t channel_count)
{
...
void avdecc_end_station_details::SetOutputChannelCount(unsigned int stream_index, size_t channel_count)
{
	switch (channel_count)
	{
	case 1:
...
void avdecc_end_station_details::UpdateChannelCount()
{
	//disregard initial mappings?
	...
	CheckInputStreamGridChanges(); //See if a channel count change fixed a dupliate mapping offset error
	input_stream_grid->ForceRefresh();
	output_stream_grid->ForceRefresh();
	CheckValid();
}

void avdecc_end_station_details::CreateInputStreamGridHeader()
{
...
void avdecc_end_station_details::CreateOutputStreamGridHeader()
{
...
void avdecc_end_station_details::CreateAndSizeStreamGrids()
{
	...
	input_stream_grid->DisableDragGridSize();
	input_stream_name_grid->EnableGridLines();
	input_stream_name_grid->DisableDragGridSize();
	...
	output_stream_grid->DisableDragGridSize();
	output_stream_name_grid->EnableGridLines();
	output_stream_name_grid->DisableDragGridSize();
	...
	dialog_sizer->Add(avdecc_end_station_details_sizer);
	dialog_sizer->Add(input_stream_label_sizer);
	dialog_sizer->Add(output_stream_label_sizer);
	dialog_sizer->Add(button_sizer);
}

unsigned int avdecc_end_station_details::index_to_channel_count(unsigned int index)
{
	switch (index)
	{
	case 0:
...
void avdecc_end_station_details::OnOK()
{
	...
	for (unsigned int i = 0; i < m_stream_input_count; i++)
	{
		struct avdecc_stream_configuration_details input_stream_details;
	...
	for (unsigned int i = 0; i < m_stream_output_count; i++)
	{
		struct avdecc_stream_configuration_details output_stream_details;
...
void avdecc_end_station_details::OnGridChange(wxGridEvent &event)
{
	...
	if (id == INPUT_GRID_ID)
	{
		...
		if (val == 0)
		{
			for (int i = selected_col; i < 8; i++) //auto clear out channels > selected channel
			{
				if (input_stream_grid->GetCellBackgroundColour(selected_row, i) != my_grey)
				{
					input_stream_grid->SetCellBackgroundColour(selected_row, i, *wxWHITE);
					input_stream_grid->SetCellValue(selected_row, i, wxT("0"));
				}
			}
		}
		...
		{
			CheckInputStreamGridChanges(); //check for duplicate cluster offsets
		}
	}
	else if (id == OUTPUT_GRID_ID)
	{
		...
		if (val == 0)
		{
			for (int i = selected_col; i < 8; i++) //auto clear out channels > selected channel
			{
				if (output_stream_grid->GetCellBackgroundColour(selected_row, i) != my_grey)
				{
					output_stream_grid->SetCellBackgroundColour(selected_row, i, *wxWHITE);
					output_stream_grid->SetCellValue(selected_row, i, wxT("0"));
				}
			}
		}
		if (val >(int)m_output_cluster_count)
		{
...
void avdecc_end_station_details::CheckInputStreamGridChanges()
{
	int num_input_rows = input_stream_grid->GetNumberRows();
	int num_input_cols = input_stream_grid->GetNumberCols();
	...
	for (int row = 0; row < num_input_rows; row++)
	{
		for (int col = 0; col < num_input_cols; col++)
		{
			if (wxAtoi(input_stream_grid->GetCellValue(row, col)) != 0)
			{
				...
				{
					int duplicate_offset = *it;
					for (int row = 0; row < num_input_rows; row++)
					{
						for (int col = 0; col < num_input_cols; col++)
						{
							if (wxAtoi(input_stream_grid->GetCellValue(row, col)) == duplicate_offset)
							{
...
int avdecc_end_station_details::ConvertClusterOffsetToAvdecc(uint16_t user_cluster_offset, uint16_t &avdecc_cluster_offset, wxString stream_type)
{
	if (stream_type.IsSameAs("STREAM_INPUT"))
	{
		...
		{
			std::cout << __FUNCTION__ << " user offset out of bounds" << std::endl;
		}
	}
	else
	{
...
void avdecc_end_station_details::CheckValid()
{
	invalid = false;
...
void avdecc_end_station_details::OnChannelChange(wxCommandEvent &event)
{
	UpdateChannelCount();
}
