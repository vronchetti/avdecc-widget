...
/**
* avdecc_end_station_configuration.cpp
*
*/

#include "avdecc_end_station_configuration.h"

avdecc_end_station_configuration::avdecc_end_station_configuration(avdecc_lib::end_station * end_station, avdecc_lib::system * sys)
{
...
avdecc_end_station_configuration::~avdecc_end_station_configuration() {}

int avdecc_end_station_configuration::GetEndStationDetails()
{
	...
	entity_id = wxT("0x");
	entity_id.Append(wxString::Format("%llx", m_end_station->entity_id()).MakeUpper());

	mac = (wxString::Format("%llx", m_end_station->mac()).MakeUpper());
	...
	if (clk_domain_desc)
	{
		...
		current_clock_source = clk_domain_resp_ref->get_clock_source_by_index(clk_domain_resp_ref->clock_source_index());
...
int avdecc_end_station_configuration::set_sample_rate(uint32_t sampling_rate)
{
	dialog_sample_rate = sampling_rate;
	return 0;
}

int avdecc_end_station_configuration::set_clock_source(uint16_t clock_source_index)
{
	dialog_clock_source = clock_source_index;
	return 0;
}

int avdecc_end_station_configuration::set_entity_name(wxString entity_name)
{
	dialog_name = entity_name;
	return 0;
}

int avdecc_end_station_configuration::SetSamplingRate()
{
...
int avdecc_end_station_configuration::cmd_set_sampling_rate(uint32_t new_sampling_rate)
{
	avdecc_lib::entity_descriptor *entity;
	avdecc_lib::configuration_descriptor *configuration;
	get_current_entity_and_descriptor(m_end_station, &entity, &configuration);
...
int avdecc_end_station_configuration::SetEntityName()
{
...
int avdecc_end_station_configuration::cmd_set_name(std::string desc_name, uint16_t desc_index, std::string new_name)
{
...
int avdecc_end_station_configuration::SetClockSource()
{
	if (current_clock_source != dialog_clock_source)
	{
...
int avdecc_end_station_configuration::cmd_set_clock_source(uint16_t new_clk_src_index)
{
	avdecc_lib::entity_descriptor *entity;
	avdecc_lib::configuration_descriptor *configuration;
...
int avdecc_end_station_configuration::cmd_display_desc_name(avdecc_lib::descriptor_base *desc, uint16_t name_index, bool is_entity)
{
	uint8_t * name;
	avdecc_lib::entity_descriptor *entity;
	avdecc_lib::configuration_descriptor *configuration;
...
int avdecc_end_station_configuration::get_current_entity_and_descriptor(avdecc_lib::end_station *end_station,
...
uint32_t avdecc_end_station_configuration::get_next_notification_id()
{
	return (uint32_t)notification_id++;
}

int avdecc_end_station_configuration::get_current_end_station_entity_and_descriptor(avdecc_lib::end_station **end_station,
	avdecc_lib::entity_descriptor **entity, avdecc_lib::configuration_descriptor **configuration)
{
